"""Windsurf configurator for OMN1 MCP integration."""
...
class WindsurfConfigurator:
    """Configure Windsurf (Codeium IDE) for OMN1 MCP server."""
    ...
    def get_config_path(self) -> Path:
        """
        Get Windsurf MCP config path based on OS.

        Returns:
            Path to Windsurf MCP config file
        """
    ...
    def get_omnimemory_prompt(self) -> str:
        """
        Get the OmniMemory usage prompt for windsurf rules.
...
"""
    ...
    def inject_windsurf_rules(self) -> Optional[Path]:
        """
        Create or update .windsurfrules file with OmniMemory prompts.

        Returns:
            Path to .windsurfrules file if updated, None if already present
        """
        windsurfrules_path = Path.home() / ".windsurfrules"

        try:
            # Check if prompts already exist
            if windsurfrules_path.exists():
                with open(windsurfrules_path, "r", encoding="utf-8") as f:
                ...
                backup_path = Path.home() / f".windsurfrules.backup-{timestamp}"
            ...
            if windsurfrules_path.exists():
                ...
                with open(windsurfrules_path, "a", encoding="utf-8") as f:
                    f.write("\n\n---\n\n")
                    f.write(prompt_text)
            else:
                # Create new file
            ...
            print(f"Warning: Could not inject .windsurfrules prompts: {e}")
            return None

    def inject_project_windsurfrules(
        self, project_path: Optional[Path] = None
    ) -> Optional[Path]:
        """
        Create or update project-specific .windsurfrules file with OmniMemory prompts.
        ...
        try:
            ...
            if windsurfrules_path.exists():
                ...
                # Backup existing file
                timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
                backup_path = project_path / f".windsurfrules.backup-{timestamp}"
                backup_path.write_text(existing_content, encoding="utf-8")
            ...
            # Silently fail - don't break the configuration process
            print(f"Warning: Could not inject project .windsurfrules prompts: {e}")
            return None

    def configure(self) -> Path:
        """
        Configure Windsurf with MCP server and inject OmniMemory prompts.
        ...
        config["mcpServers"]["omn1"] = {
            ...
            "env": {
                "OMNIMEMORY_TOOL_ID": "windsurf"  # Identifies which tool is using OmniMemory
            },
        }
        ...
        # Inject OmniMemory prompts into .windsurfrules
        windsurfrules_path = self.inject_windsurf_rules()
        if windsurfrules_path:
            print(f"✅ Updated {windsurfrules_path}")
        else:
            print("ℹ️  .windsurfrules already contains OmniMemory prompts")
