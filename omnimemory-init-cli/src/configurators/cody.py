"""Cody (Sourcegraph) configurator for OMN1 MCP integration."""
...
class CodyConfigurator:
    """Configure Cody (Sourcegraph) for OMN1 MCP server via OpenCtx."""
    ...
    def get_omnimemory_prompt(self) -> str:
        """
        Get the OmniMemory usage prompt to embed in Cody config.
...
"""
    ...
    def configure(self) -> Path:
        """
        Configure Cody (via VS Code settings) with MCP server through OpenCtx and embed OmniMemory prompts.
        ...
        # Enable OpenCtx
        config["openctx.enable"] = True

        # Ensure openctx.providers key exists
        if "openctx.providers" not in config:
            config["openctx.providers"] = {}

        # Add OMN1 MCP server via OpenCtx provider
        provider_key = "https://openctx.org/npm/@openctx/provider-modelcontextprotocol"
        config["openctx.providers"][provider_key] = {
            "nodeCommand": self.venv_python,
            "mcp.provider.uri": f"file://{self.mcp_script}",
            "mcp.provider.args": [],
            "mcp.provider.env": {
                "OMNIMEMORY_TOOL_ID": "cody"  # Identifies which tool is using OmniMemory
            },
        }

        # Embed OmniMemory prompt in Cody's systemPrompt field
        config["cody.systemPrompt"] = self.get_omnimemory_prompt()
