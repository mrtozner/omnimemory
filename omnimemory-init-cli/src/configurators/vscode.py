"""VSCode configurator for OMN1 MCP integration."""
...
class VSCodeConfigurator:
    """Configure VSCode (Cline extension) for OMN1 MCP server."""
    ...
    def get_config_path(self) -> Path:
        """
        Get VSCode user settings path based on OS.
    ...
    def get_cline_mcp_config_path(self) -> Path:
        """
        Get VSCode Cline MCP config path for MCP server configuration.

        Returns:
            Path to VSCode Cline MCP config file
        """
        # Same path for all platforms
        return Path.home() / ".vscode" / "cline_mcp_settings.json"

    def get_omnimemory_prompt(self) -> str:
        """
        Get the OmniMemory usage prompt to embed in VSCode config.
...
"""
    ...
    def configure(self) -> Path:
        """
        Configure VSCode Cline with MCP server and embed OmniMemory prompts.

        Returns:
            Path to config file that was modified
        """
        # Configure MCP server in Cline's MCP settings
        mcp_config_path = self.get_cline_mcp_config_path()
        ...
        if mcp_config_path.exists():
            try:
                ...
                print(f"Warning: Could not read MCP config: {e}")
                mcp_config = {}
        else:
            mcp_config = {}
        ...
        # Add OMN1 MCP server
        mcp_config["mcpServers"]["omn1"] = {
            "command": self.venv_python,
            "args": [self.mcp_script],
            "env": {
                "OMNIMEMORY_TOOL_ID": "vscode-copilot"  # Identifies which tool is using OmniMemory
            },
        }
        ...
        print(f"✅ Configured MCP server in {mcp_config_path}")

        # Configure prompts in VS Code settings
        ...
        # Embed OmniMemory prompt in Cline's customInstructions field
        settings["cline.customInstructions"] = self.get_omnimemory_prompt()
        ...
        print(f"✅ Embedded OmniMemory prompts in {settings_path}")
