"""Continue.dev configurator for OMN1 MCP integration."""
...
class ContinueConfigurator:
    """Configure Continue.dev for OMN1 MCP server."""
    ...
    def get_config_path(self) -> Path:
        """
        Get Continue.dev main config path.

        Returns:
            Path to Continue config.json file
        """
    ...
    def get_omnimemory_prompt(self) -> str:
        """
        Get the OmniMemory usage prompt to embed in Continue config.
...
"""
    ...
    def configure(self) -> Path:
        """
        Configure Continue.dev with MCP server and embed OmniMemory prompts.

        Returns:
            Path to config file that was created/modified
        """
        ...
        config["mcpServers"]["omn1"] = {
            ...
            "env": {
                "OMNIMEMORY_TOOL_ID": "continue-dev"  # Identifies which tool is using OmniMemory
            },
        }

        # Embed OmniMemory prompt in Continue's systemMessage field
        config["systemMessage"] = self.get_omnimemory_prompt()
